...
class UBody(Dataset):
    ...
    def print_eval_single(self, eval_result, key):
        mean = np.mean(eval_result[key])
        min_err = np.min(eval_result[key])
        max_err = np.max(eval_result[key])
        std = np.std(eval_result[key])
        med = np.median(eval_result[key])
        print(f'{key}: {mean}, min {min_err}, max {max_err}, std {std}, median {med}')

    def print_eval_result(self, eval_result):
        for k in eval_result.keys():
            self.print_eval_single(eval_result, k)
        return
